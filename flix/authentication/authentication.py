...
def login():
    ...
    if form.validate_on_submit():
        try:
            ...
            password_does_not_match_username = 'Password does not match username - please try again'
...
class PasswordValid:
    def __init__(self, message = None):
        if not message:
            message = u'Your password must be at least 8 characters, and contian an upper case letter, a lower case letter and a digit'
...
class RegistrationForm(FlaskForm):
    username = StringField('Username', [DataRequired(message = 'Username is required'), Length(min = 3, message = 'Username must have 3 or more characters')])
    password = PasswordField('Password', [DataRequired(message = 'Password is required'), PasswordValid()])
